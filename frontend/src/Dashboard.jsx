...
function StatCard({ title, value, subtitle, icon, accent }) {
  return (
    ...
    >
      <div style={{ ...styles.statIcon, color: accent, background: `${accent}14` }}>
        {icon}
      </div>
...
function getViolationType(row) {
  ...
  if (lower.includes("helmet")) return "Helmetless Riding";
  if (lower.includes("triple") || lower.includes("rider")) return "Excessive Rider Count";
  if (lower.includes("mobile") || lower.includes("phone")) return "Mobile Phone Usage";
  if (lower.includes("wrong")) return "Wrong-Way Movement";
...
function getSeverity(type) {
  ...
  if (lower.includes("wrong")) return "danger";
  if (lower.includes("helmet")) return "warning";
  if (lower.includes("mobile")) return "warning";
  if (lower.includes("rider") || lower.includes("triple")) return "danger";
...
export default function Dashboard() {
  ...
  const normalized = useMemo(() => {
    return violations.map((row, index) => ({
      ...row,
      _id: index,
      _type: getViolationType(row),
      _severity: getSeverity(getViolationType(row)),
  ...
  const filteredViolations = useMemo(() => {
    return normalized.filter((row) => {
      const matchesFilter =
        filter === "All" ||
        row._type.toLowerCase().includes(filter.toLowerCase());
  ...
  const chartData = [
    { label: "Helmet", value: counts.helmet },
    { label: "Rider", value: counts.rider },
    { label: "Mobile", value: counts.mobile },
    { label: "Wrong Way", value: counts.wrongWay },
  ];

  const maxChartValue = Math.max(...chartData.map((x) => x.value), 1);

  const videoUrl = `${API_BASE}/videos/helmetless_fixed.mp4`;

  function snapshotUrl(row) {
    if (!row?._snapshot) return "";
    ...
    <div style={styles.app}>
      ...
      <aside style={styles.sidebar}>
        <div style={styles.brand}>
          ...
          <div>
            ...
            <div style={styles.brandSub}>AI MOBILITY INTELLIGENCE</div>
          </div>
        </div>
        ...
        {[
          ...
          >
            ...
            {item === "Violations" && normalized.length > 0 && (
              <span style={styles.navCount}>{normalized.length}</span>
            )}
          </button>
        ))}
        ...
        <div style={styles.sidebarBottom}>
          <div style={styles.systemBox}>
            ...
            <div style={styles.systemText}>
              Edge-AI perception and mobility data service
            </div>
          </div>
      ...
      <main style={styles.main}>
        ...
        <header style={styles.header}>
          <div>
            <div style={styles.breadcrumb}>ROADSENSE / {activeNav.toUpperCase()}</div>
            <h1 style={styles.pageTitle}>Mobility Intelligence Center</h1>
            <p style={styles.pageSubtitle}>
              Edge-AI powered traffic monitoring and violation intelligence
            </p>
          </div>
          ...
          <div style={styles.headerRight}>
            <div style={styles.liveStatus}>
              ...
              />
              {backendOnline ? "LIVE SYSTEM" : "OFFLINE"}
            </div>

            <button style={styles.refreshButton} onClick={fetchViolations}>
              ↻ Refresh
            </button>
          </div>
        </header>
        ...
        <section style={styles.hero}>
          <div>
            <div style={styles.heroEyebrow}>EDGE-AI TRAFFIC OPERATIONS</div>
            ...
            <p style={styles.heroDescription}>
              RoadSense processes CCTV intelligence into structured mobility
              events, evidence, analytics and actionable traffic insights.
            </p>

            <div style={styles.heroPills}>
              <span style={styles.heroPill}>YOLOv8</span>
              <span style={styles.heroPill}>ByteTrack</span>
              <span style={styles.heroPill}>Edge AI</span>
              <span style={styles.heroPill}>ANPR</span>
            </div>
          </div>
          ...
          <div style={styles.heroOrb}>
            <div style={styles.orbRing}>
              <div style={styles.orbCore}>
                <div style={styles.orbNumber}>{counts.total}</div>
                <div style={styles.orbLabel}>EVENTS</div>
              </div>
            </div>
          </div>
        </section>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.chart}>
              {chartData.map((item, index) => {
                ...
                return (
                  <div style={styles.barGroup} key={item.label}>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.statusList}>
              ...
              <div style={styles.statusRow}>
                ...
                <Badge type={backendOnline ? "success" : "danger"}>
            ...
            <div style={styles.healthMeter}>
              <div style={styles.healthHeader}>
                ...
                <strong>{backendOnline ? "Operational" : "Attention"}</strong>
              </div>
        ...
        <section style={styles.twoColumn}>
          <div style={styles.card}>
            <SectionHeader
              ...
              action={<Badge type="success">READY</Badge>}
            />
        ...
        <section style={styles.card}>
          ...
          {loading ? (
            <div style={styles.emptyState}>
              ...
              <div>Synchronizing mobility intelligence...</div>
            </div>
            ...
            <div style={styles.emptyState}>
              ...
              <div style={styles.emptyText}>
                No traffic violations match the current filter.
              </div>
            </div>
          ) : (
            <div style={styles.tableWrap}>
              <table style={styles.table}>
                ...
                <tbody>
                  {filteredViolations.slice(0, 50).map((row) => (
                    <tr key={row._id} style={styles.tableRow}>
                        ...
                        <div style={styles.eventCell}>
                          <div
                            style={{
                              ...styles.eventDot,
                              background:
                                row._severity === "danger"
                                  ? COLORS.red
                                  : row._severity === "warning"
                                  ? COLORS.yellow
                                  : COLORS.cyan,
                            }}
                          />
                          ...
                          <div>
                            <div style={styles.eventName}>{row._type}</div>
                      ...
                      <td style={styles.mutedCell}>{row._timestamp}</td>

                      <td style={styles.vehicleCell}>{row._vehicle}</td>

                      <td>
                        <span style={styles.plate}>{row._plate}</span>
                      </td>

                      <td>
                        <Badge type={row._severity}>
        ...
        <footer style={styles.footer}>
          <div>
            <strong>RoadSense AI</strong>
            <span> · Edge-AI Based Mobility Intelligence Platform</span>
          </div>

          <div>
            {backendOnline ? "● System operational" : "● Backend unavailable"}
          </div>
        </footer>
      </main>
        ...
        >
          ...
          >
            <div style={styles.modalHeader}>
              <div>
                <div style={styles.modalEyebrow}>TRAFFIC EVENT</div>
                <h2 style={styles.modalTitle}>{selected._type}</h2>
              </div>
            ...
            <div style={styles.modalContent}>
              ...
              <div style={styles.evidenceInfo}>
                <div>
                  ...
                  <strong>{selected._type}</strong>
                </div>

                <div>
                  <span>TIMESTAMP</span>
                  <strong>{selected._timestamp}</strong>
                </div>

                <div>
                  <span>VEHICLE</span>
                  <strong>{selected._vehicle}</strong>
                </div>

                <div>
                  <span>NUMBER PLATE</span>
                  <strong>{selected._plate}</strong>
                </div>
              </div>
            </div>
          </div>
...
const styles = {
  ...
  navButtonActive: {
    color: "#f8fafc",
    background: "linear-gradient(90deg, rgba(34,211,238,0.13), rgba(34,211,238,0.04))",
